...
k_d1 = eval(stringent_sys_params["k_d1"])
...
def solve_qcrispri(alphaT = alpha0, alphaIac = alphaI,  activity_ratio=0.1, Kd1 = kd1, Kd2 = kd2, gfp0 = 1442.00126, t_start_decay=3.0):

    #use 50% as default to study + and - variation
    alphaG_deac = alphaG*activity_ratio
    kd1 = Kd1
    kd2 = Kd2
    y0=[pT, 0, 10**7, 0, 0, 0, 1442.00126, gfp0, 0, pT, 0]
    alpha = [alpha0, alphaIac, alphaR, alphaG, alphaG_deac, alphaT]
    n = nA
    kd = [k1R, k2R,kcasR, kd1, kd2, kdCas, k_d]
    deg = [dR, dA, dG, dCas, dc]
    miscc = [r, K, Vc, pT, mu, m, c]
    t_start_decay=3.0

    # Solve ODEs for "qCRISPRi" 
    sol = odeint(ode_gen_qcrispri, y0, t, args=(alpha, n, kd, deg, miscc, t_start_decay))
    cols = ['inAcP', 'AcP', 'N', 'LuxR', 'AHL', 'LuxR_A', 'GFPc', 'GFP', 'dCas9', 'pC', 'pD']
    return(convert2df(sol, cols))



def solve_qcrispri_simple(alphaRL = alpha_rl, alphaRA = alpha_ra, nH_AHL = nHill_A, KD = Kd, rs = r1, t_start_decay=4.5):
    
    y0=[0, 0, 1726.752756, 10**7]
    alpha = [alpha_pl, alphaRL, alphaRA, alpha_g]
    n = [nH_AHL, nHill_c, nAHL]
    kd = [KD, KdCas]
    deg = [Kdeg_A, Kdeg_Cas, Kdeg_G,k_d1]
    miscc = [rs, K1, Vc, m1, c1]

    # Solve ODEs for "qCRISPRi" 
    sol = odeint(ode_gen_qcrispri_simple, y0, t, args=(alpha, n, kd, deg, miscc, t_start_decay))
